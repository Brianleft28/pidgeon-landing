...
const PayPalLogo = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="PayPal Logo">
    <path d="M19.64 5.92c-.36-1.57-1.63-2.38-3.79-2.38H6.5c-.5 0-.91.37-.99.86L2.68 20.9c-.04.25.15.48.41.48h3.33c.4 0 .74-.29.81-.68l.9-5.71h2.57c3.9 0 6.64-1.92 7.37-6.23.09-.54.12-1.04.12-1.51 0-2.32-.98-3.74-2.55-4.33z"/>
    <path d="M17.06 6.3c-.36-1.57-1.63-2.38-3.79-2.38H6.5c-.5 0-.91.37-.99.86L4 14.5h2.5l.65-4.14c.07-.4.41-.68.81-.68h3.75c3.55 0 6.09-1.74 6.78-5.32.08-.43.12-.86.12-1.28 0-1.89-.62-3.08-1.55-3.78z" className="opacity-40"/>
  </svg>
);

const StripeLogo = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Stripe/Card Logo">
    <path d="M2 9h20v10.5a2.5 2.5 0 01-2.5 2.5h-15a2.5 2.5 0 01-2.5-2.5V9zm10-7C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z" className="opacity-0" /> 
...
export const PaymentModal: React.FC<PaymentModalProps> = ({ isOpen, onClose, planName, amount }) => {
  ...
  const [selectedMethod, setSelectedMethod] = useState<'mp' | 'paypal' | 'card'>('mp');
  ...
  const handlePayment = () => {
      ...
      alert(`Initiating payment for ${planName} ($${amount}) via ${selectedMethod.toUpperCase()}`);
      onClose();
  };
  ...
  const methods = [
    { 
        id: 'mp', 
        label: t.paymentModal.methods.mp, 
    ...
    { 
        id: 'paypal', 
        label: t.paymentModal.methods.paypal, 
        Logo: PayPalLogo,
        colorClass: 'text-blue-600', 
        bgClass: 'bg-blue-600/10', 
        borderClass: 'border-blue-600',
        disabled: true,
        tooltip: "En desarrollo / Coming Soon"
    },
    { 
        id: 'card', 
        label: "Stripe / Card", 
  ...
  const handleMethodKeyDown = (e: React.KeyboardEvent, index: number) => {
    ...
    if (nextIndex !== -1 && !methods[nextIndex].disabled) {
      setSelectedMethod(methods[nextIndex].id as any);
  ...
  return (
    ...
    >
      <div 
        ref={modalRef}
        className="bg-[#0f172a] border border-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-slideUp relative"
        onClick={(e) => e.stopPropagation()}
      >
        <button 
            ref={closeButtonRef}
            onClick={onClose} 
            className="absolute top-4 right-4 text-slate-500 hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 rounded-full p-1"
        ...
        <div className="p-8 text-center border-b border-slate-800 bg-slate-900/50">
            <h3 id="payment-modal-title" className="text-xl font-bold text-white mb-1">{t.paymentModal.title}</h3>
            <p className="text-xs text-slate-400 uppercase tracking-wide">{t.paymentModal.subtitle}</p>
        </div>

        <div className="p-6">
            <div className="flex justify-between items-center bg-slate-900 rounded-lg p-4 mb-6 border border-slate-800">
                <div>
                    <span className="block text-xs text-slate-500 uppercase">{t.paymentModal.selectedPlan}</span>
                    <span className="font-bold text-white">{planName}</span>
                </div>
                <div className="text-right">
                    <span className="block text-xs text-slate-500 uppercase">{t.paymentModal.total}</span>
                    <span className="font-bold text-emerald-400 text-xl" aria-label={`${amount} US Dollars`}>US$ {amount}</span>
                </div>
            </div>

            <div className="space-y-3 mb-8" role="radiogroup" aria-label="Select Payment Method">
                {methods.map((method, index) => {
                    ...
                    return (
                        <div key={method.id} className="relative group/tooltip">
                            <button 
                                ...
                                disabled={isDisabled}
                                onClick={() => !isDisabled && setSelectedMethod(method.id as any)}
                                onKeyDown={(e) => !isDisabled && handleMethodKeyDown(e, index)}
                                className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-slate-900 group ${
                                    isDisabled 
                                        ? 'bg-slate-900/50 border-slate-800 opacity-50 cursor-not-allowed grayscale' 
                                        : isSelected 
                                            ? `${method.bgClass} ${method.borderClass} shadow-lg` 
                            ...
                            >
                                <span className="flex items-center gap-4 text-white font-medium">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center overflow-hidden ${isSelected && !isDisabled ? 'bg-white/10' : 'bg-slate-800 group-hover:bg-slate-700'}`}>
                                        <Logo className={`w-full h-full p-2 ${isSelected && !isDisabled ? method.colorClass : 'text-slate-400 group-hover:text-white'} transition-colors`} />
                                    </div>
                                    <span>{method.label}</span>
                                </span>
                                
                                <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${
                                    isDisabled ? 'border-slate-700' : 
                                    isSelected 
                                        ? `border-current ${method.colorClass}` 
                                        : 'border-slate-600 group-hover:border-slate-500'
                                }`}>
                                    {isSelected && !isDisabled && <div className={`w-2.5 h-2.5 rounded-full bg-current`} />}
                                </div>
                            </button>
                            {/* Tooltip for Disabled State */}
                            {isDisabled && method.tooltip && (
                                <div role="tooltip" className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-[200px] px-3 py-1.5 bg-slate-800 text-slate-300 text-xs font-semibold rounded-md border border-slate-700 opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none shadow-xl z-20 text-center select-none">
                                    {method.tooltip}
            ...
            <button 
                ...
                className="w-full py-4 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-lg transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] mb-4 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-slate-900"
                aria-label={`Pay ${amount} US Dollars with ${selectedMethod === 'mp' ? 'Mercado Pago' : selectedMethod === 'paypal' ? 'PayPal' : 'Credit Card'}`}
            >
                {t.paymentModal.cta}
            </button>
